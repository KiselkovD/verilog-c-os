module cache_altera #(
    ...
    parameter INIT_FILE = ""    // Initialization file (hex format)
) (
  input wire i_clk,
...
// Use generic memory for simulation (Icarus Verilog)
`ifdef SIMULATION
  // Generic memory array for simulation
  ...
  initial begin
    ...
    if (INIT_FILE != "") begin
      $display("CACHE_ALTERA: Loading initialization file: %s", INIT_FILE);
      $readmemh(INIT_FILE, mem);
    end else begin
      $display("CACHE_ALTERA: No initialization file specified");
    end
  end
  ...
  // Output is the data from memory
  assign o_data = i_request ? mem[i_address] : {DATA_WIDTH{1'bz}};

`else
  // Use Altera altsyncram for synthesis
