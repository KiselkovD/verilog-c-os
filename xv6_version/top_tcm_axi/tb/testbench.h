...
public:
    ...
    testbench(sc_module_name name): testbench_vbase(name)
    {
		...
		// Enable Verilator tracing after elaboration but before simulation
		SC_THREAD(enable_trace_after_elaboration);
    }

	// Thread to enable trace after elaboration
	void enable_trace_after_elaboration()
	{
		wait(SC_ZERO_TIME); // Wait for elaboration to complete
		if (waves_enabled()) {
			Verilated::traceEverOn(true);
			VerilatedVcdC *vcd = new VerilatedVcdC;
			m_dut->trace_enable(vcd);
			vcd->open("verilator.vcd");
			m_verilate_vcd = vcd;
		}
	}
    //-----------------------------------------------------------------
    ...
    // Method to enable Verilator tracing after simulation starts
    void enable_verilator_tracing()
    {
        if (waves_enabled()) {
            Verilated::traceEverOn(true);
            VerilatedVcdC *vcd = new VerilatedVcdC;
